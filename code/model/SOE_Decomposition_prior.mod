...
// Spending parameters  
rho_g_i   =     0.475066770320351; 
alpha_g_i =     1.77181463858898;         
gamma_g_i =     -0.128020819907481; 
sig_g_i   =     0.152928493631706;  

rho_g_c   =     0.700008971370747;
alpha_g_c =     0.133003777051124; 
gamma_g_c =     -0.00559524541775913; 
sig_g_c   =     0.0270612481674496;     

rho_TR    =     0.371415315040748;
alpha_TR  =     3.51994486226213;      
gamma_TR  =     -0.348167302489475; 
sig_TR    =     0.0975543556649158;

 
// Deep Parameters
phi          =   0.1;      
varsigma     =   0.9; 
hpsi         =   0.5;             % Posterior from Guerra-Salas et al.
eta          =   3; 
gamma_k      =   6;
eta_c        =   1.5;
upsilon      =   0.5;
...
sig_v        =	1;
sig_kappa    =	4;              % To match volatility of hours
sig_u	     =	1;              % To match volatility of output
sig_z        =	0.1;            % To match volatility of output
sig_a        =	0.1;            % To match volatility of output
sig_zeta     =	0.9;            % To match volatility of country risk
...
//Deep parameters
phi,                , 0, inf, normal_pdf, 2, 2;       // Inverse Frisch Elasticity. Prior from Guerra-Salas et al (2021)
varsigma,           ,  ,    , beta_pdf, 0.8, 0.1;     // Habit formation. Posterior from Guerra-Salas et al (2021)   
hpsi,               , 0, inf, normal_pdf, 1, 10;       // 100 Times Elasticity of Country Premium. Prior from Guerra-Salas et al (2021)
eta,                , 0, inf, normal_pdf, 1.5, .5;    // Elasticity of Substitution Between Home and Foreign Goods. Prior from Guerra-Salas et al (2021)
...
eta_c,              , 0, inf, normal_pdf, 0.5, .5;    // Elasticity of Substitution Between Private and Government Consumption. Posterior from Coenen et al (2013)
upsilon,            ,  ,    , beta_pdf, 0.3, 0.1;     // Preference shifter controlling income effect on labor. Posterior from Guerra-Salas et al (2021)

//AR coefficients
rho_v,               beta_pdf, 0.75, 0.1;    // Prior from Guerra-Salas et al (2021)  
rho_kappa,           beta_pdf, 0.75, 0.1;    // Prior from Guerra-Salas et al (2021) 
rho_u,               beta_pdf, 0.75, 0.1;    // Prior from Guerra-Salas et al (2021) 
rho_z,               beta_pdf, 0.75, 0.1;    // Prior from Guerra-Salas et al (2021) 
rho_a,               beta_pdf, 0.75, 0.1;    // Prior from Guerra-Salas et al (2021) 
rho_zeta,            beta_pdf, 0.75, 0.1;    // Prior from Guerra-Salas et al (2021) 
   

// Innovation std. deviations
stderr eps_v,           inv_gamma_pdf, .1, inf;    // Prior from Guerra-Salas et al (2021)
stderr eps_kappa,       inv_gamma_pdf, .1, inf;    // Prior from Guerra-Salas et al (2021)  
stderr eps_u,           inv_gamma_pdf, .1, inf;    // Prior from Guerra-Salas et al (2021)  
stderr eps_z,           inv_gamma_pdf, .1, inf;    // Prior from Guerra-Salas et al (2021) 
stderr eps_a,           inv_gamma_pdf, .1, inf;    // Prior from Guerra-Salas et al (2021) 
stderr eps_zeta,        inv_gamma_pdf, .1, inf;    // Prior from Guerra-Salas et al (2021)

//Fiscal parameters
rho_g_c,             beta_pdf,  0.695335602121962, 0.15;                          // Prior mean from external estimation (significant)
stderr eps_g_c,      inv_gamma_pdf, 0.0270415156952085, inf;                      // Prior mean from external estimation 
alpha_g_c,           normal_pdf, 0.420787820063673, 7;                            // Prior from external estimation (not significant)
gamma_g_c,           -0.052205750944297,-inf,0,normal_pdf,-0.052205750944297, 1;  // Prior from external estimation (not significant)

rho_g_i,              beta_pdf,   0.472291197161569, 0.25;                         // Prior mean from external estimation (significant)
stderr eps_g_i,       inv_gamma_pdf,0.153201302813339, inf;                        // Prior mean from external estimation 
alpha_g_i,            normal_pdf,  3.373052046866841, 7;                           // Prior from external estimation (significant)
gamma_g_i,            -0.021234410874796,-inf,0,normal_pdf,-0.021234410874796, 1;  // Prior from external estimation (not significant)

rho_TR,              beta_pdf,  0.380757720735125, 0.2;                            // Prior mean from external estimation (significant)
stderr eps_TR,       inv_gamma_pdf, 0.0998494301876969, inf;                       // Prior mean from external estimation 
alpha_TR,            normal_pdf, 5.603666220396844, 7;                             // Prior from external estimation (significant)
gamma_TR,            -0.364600642998102,-inf,0,normal_pdf,-0.364600642998102, 1;   // Prior from external estimation (not significant)
...
estimated_params_init; // From fmincon (mode_compute=1)
...
gamma_TR,	        -0.0004;
stderr eps_v,	    0.0409;
stderr eps_kappa,	0.0224;
stderr eps_u,	    0.0556;
stderr eps_z,	    0.0224;
stderr eps_a,	    0.0217;
stderr eps_zeta,	0.0130;
stderr eps_g_c,	    0.0158;
stderr eps_g_i,	    0.1151;
stderr eps_TR,	    0.2124;
...
//estimation(datafile='/mq/manuel_projects/SOEM_EC/data/estim_data.xlsx', mode_compute=6, mh_replic=500000, mh_jscale=.25, mh_nblocks=1, mh_drop=0.5, plot_priors=0, cova_compute=1);
//estimation(datafile='/mq/manuel_projects/SOEM_EC/data/estim_data.xlsx', mode_file = 'SOE_Estimation_mh_mode.mat', mode_compute=0, mh_replic=500000, mh_jscale=0.25, mh_nblocks=2, mh_drop=0.5, plot_priors=0);
...
//shock_decomposition(datafile='/mq/manuel_projects/SOEM_EC/data/estim_data.xlsx',parameter_set=calibration, use_shock_groups, colormap=mymap, nograph) gam_YNCo_obs gam_C_obs gam_I_obs hours_obs
